...
plot_style = 'darkgrid'
plot_context = 'paper'
sns.set(context = plot_context, style = plot_style)
...
#Playing with bar charts to see what the data looks like. 
...
# The data represents Chile, China, Germany, Mexico, USA, and Zimbabwe

# What years are represented in the data?

# The GDP and Life Expectancy data range from 2000 to 2015. 
...
fig1, ax1 = plt.subplots(figsize=(15,15), ncols=1, nrows=2, sharex = True)
bar_gdp = sns.barplot(data = df, x = 'Country', y = 'GDP', ax = ax1[0])
bar_gdp.xaxis.set_visible(False)
bar_leaby = sns.barplot(data = df, x = 'Country', y = 'LEABY', ax = ax1[1])
#Zimbabwe is too small to see. 
bar_gdp.annotate('2015: ~ 1.6 Milliion USD', xy=(5, 1), xytext=(4.5, 00.25e13), arrowprops=dict(facecolor='black', shrink=0.15))
#Make USA fit
bar_leaby.set_xticklabels(['Chile','China','Germany','Mexico','U.S.A.','Zimbabwe'])
plt.xticks(rotation = 90)
plt.show()
...
#created both charts above. 


# What do you notice about the two bar charts? Do they look similar?

# The two charts are very different. The data for Zimbabwe doesnt even fit on the graph. 
...
ax2 = sns.violinplot(data = df, x = 'Country', y = 'LEABY')
#Make USA fit so it looks good in print.
ax2.set_xticklabels(['Chile','China','Germany','Mexico','U.S.A.','Zimbabwe'])
plt.ylabel('Life Expectancy at Birth (years)', fontsize=14, fontweight='bold')
plt.xlabel('Country', fontsize=14, fontweight='bold')
plt.title('Life Expectancy from 2000 to 2015 by Country', fontweight='bold', fontsize=18)
...
#  Zimbabwe had the greatest change, but the other countries are mostly within the same range. 
...
ax3 = sns.barplot(data = df, x = 'Country', y = 'GDP', hue = 'Year')
ax3.set(ylabel = 'GDP in Trillions of USD', xlabel = 'Country by Name')
ax3.set(title = 'GDP over time by Country')
#Zimbabwe is really small by comparison. 
ax3.annotate('2015: ~ 1.6 Milliion USD', xy=(5, 1), xytext=(4.5, 00.25e13), arrowprops=dict(facecolor='black', shrink=0.15))
plt.legend(loc=6)
plt.xticks(rotation = 90)
plt.show()
...
f4, ax4 = plt.subplots(figsize=(15, 10))
plt.xticks(rotation = 90)
ax4 = sns.barplot(data = df, x = 'Country', y = 'LEABY', hue = 'Year')
ax4.set(ylabel = 'Life expectancy at birth in years', xlabel = 'Country by Name')
ax4.set(title = 'Life expectancy over time by Country')
plt.legend(loc=6)
plt.show()
...
# 
# **TP** - _Zimbabwe has the greatest change from 2004 to 2015. China had considerable increase as well._
# 
# - Which country has had the least change in GDP over time? 
# 
# **TP** - _The U.S.A. seems to have the flattest growth over time._
# 
# - How do countries compare to one another?
# 
# **TP** - _With the exception of Zimbabwe, most countries are fairly simlar. _
# 
# - Now that you can see the both bar charts, what do you think about the relationship between GDP and life expectancy?
# 
# **TP** - _I dont think GDP and life expectancy are related. 
# 
# - Can you think of any reasons that the data looks like this for particular countries?
# 
# **TP** - _Access to medicine likely has more direct impact than GDP on life expectancy._
...
#Step 8
grid1 = sns.FacetGrid(df, col='Year', hue='Country', col_wrap=4, height=2)
grid1 = (grid1.map(plt.scatter, 'GDP', 'LEABY', edgecolor='w').add_legend())
plt.subplots_adjust(top=0.9)
grid1.fig.suptitle('Annual GDP compared and Life Expectancy by Country', fontsize = 18, fontweight = 'bold')
grid1.set_axis_labels(x_var='GDP (USD trillions)', y_var='Life Expectancy')
grid1.set_titles('{col_name}')
grid1.set_xticklabels([0.5, 1, 1.5])
grid1.set(yticks = [45, 55, 65, 75], xticks = [.5e13, 1e13, 1.5e13])
grid1.despine(right = False, top = False)
...
# 
# **TP** - _Zimbabwe shows the most increase on the x axis._
# 
# + Which country moves the most along the Y axis over the years?
# 
# **TP** - _China has the greatest increase on the Y axis._
# 
# + Is this surprising?
# 
# **TP** - _Not really. China and USA are wealth countries._
# 
# + Do you think these scatter plots are easy to read? Maybe there's a way to plot that! 
# 
# **TP** - _I like showing data this way, but it takes some time to analyze._
...
#Step 9
grid2 = sns.FacetGrid(df, col='Country', col_wrap=3, height=4)
grid2 = (grid2.map(plt.plot, 'Year', 'LEABY'))
grid2.set_titles('{col_name}')
grid2.fig.suptitle('Average Life Expectancy by Country', fontsize = 18, fontweight = 'bold')
grid2.set_axis_labels(x_var='Year', y_var='Life Expectancy (years)')
grid2.despine(left = True)
plt.subplots_adjust(top=0.9)
plt.xticks(range(2000, 2020, 5))
plt.savefig('Leaby_by_Country_per_year.png')
plt.show()
...
# 
# **TP** - _Zimbabwe again shows the greatest change._
# 
# - What years are there the biggest changes in the data?
# 
# **TP** - _The last ten years show the greatest change._
# 
# - Which country has had the least change in life expectancy over time? 
# 
# **TP** - _The U.S. shows the least change._
# 
# - Can you think of any reasons that the data looks like this for particular countries?
# 
# **TP** - _The history of each country is likely the reason for the data showing this way._
...
#Step 10
grid3 = sns.FacetGrid(df, col='Country', col_wrap=3, height=4, margin_titles = False)
grid3 = (grid3.map(plt.plot, 'Year', 'GDP'))
grid3.set_titles('{col_name}')
grid3.fig.suptitle('GDP by Country', fontsize = 18, fontweight = 'bold')
grid3.set_axis_labels(x_var='Year', y_var='GDP in trillions of USD')
grid3.despine(left = True)
plt.subplots_adjust(top=0.9)
plt.xticks(range(2000, 2020, 5))
plt.savefig('GDP_by_Country_per_year.png')
plt.show()


# Which countries have the highest and lowest GDP?

# **TP** - _Zimbabwe and Chile appear to be the lowest with the U.S. and China being the highest GDP._

# Which countries have the highest and lowest life expectancy?

# **TP** - _Germany has the highest life expectancy with Zimbabwe coming in as the lowest._
...
# **TP** - _The changes in foreign relations between the U.S.A and China is likely related to this change.
...
# **TP** - Blog posted here: https://medium.com/@t.e.pope/gdp-vs-life-expectancy-20df9b8540f5
